...
\", \"target\":\"action's target|destination name\", \"message\":\"lines of dialogue\"}"
        ...
        if (isset($GLOBALS["CONNECTOR"][$this->name]["stop"]) && is_array($GLOBALS["CONNECTOR"][$this->name]["stop"])) {
            ...
            foreach ($GLOBALS["CONNECTOR"][$this->name]["stop"] as $stopSeq) {
                if (is_string($stopSeq) && !empty($stopSeq)) { $validStop[] = $stopSeq; }
            }
        ...
        try {
            ...
            if($uaStatus === 'MISS' && !empty($uaReason)) { $uaCacheLog.='('.$uaReason.')'; }
                    ...
                    if (isset($data['type'])) { // Anthropic Format
                        switch ($data['type']) {
                            ...
                            case 'message_stop':
                                ...
                                $logPerfEntry = sprintf("[%s] ANTHROCACHE_JSON %s: Read:%d Create:%d New:%d TotalIn:%d Efficiency:%.1f%%\n",
    ...
    public function close() {
        ...
        try {
            ...
            $logContent = sprintf("Raw Stream Data:\n%s\n\nProcessed Text:\n%s\n\nJSON Responses:\n%s\n\n[%s] [%s:%s] END STREAM\n==\n",
                $raw, $proc, $jsonResponses, date(DATE_ATOM), $this->name, $herikaName);
    ...
    private function processAnthropicToolUse() {
        ...
        foreach ($this->_jsonResponsesEncoded as $jsonStr) {
            ...
            if (isset($data['type']) && $data['type'] === 'content_block_start' && 
